...
int ast_eval2(struct ast_tree * tree, char ** variable1, char** variable2, char ** value1,char ** value2, int * type1,int * type2, int num_of_fields1,int num_of_fields2, int* flag,char* table1,char* table2){   // evaluates the ast tree
	...
	if (tree->nodetype <= 2){    // this piece of code was written to handle the cases when strings or numbers can itself evaluate to boolean values for condition but due to conflicts in grammar it was not pursued
		...
		if (tree->left->nodetype == 2 && tree->right->nodetype == 2){   // if both the left child and right child are leafs of variable(not constant) type
			...
			int pos=find(tree->left->data.str,'.');
			char* realtable_1 = strncpy((char*)malloc(pos+1), tree->left->data.str);
			realtable_1[pos]='\0';
			pos=find(tree->right->data.str,'.');
			char* realtable_2=strncpy((char*)malloc(pos+1),tree->right->data.str);
			realtable_2[pos]='\0';
			...
			if(type_2 !=  type_1){    // type mismatch
				*(flag) = 3;
				return (0);
			}
			else {
				if(type_2 == 0){   // both are numbers
					return(evaluatenum(tree->nodetype, atoi(str1), atoi(str2), flag)) ;
				}
				else{     // both are strings
					return(evaluatestr(tree->nodetype, str1, str2, flag));
				}
			...
			if (tree->nodetype == 15){  // AND node
				return(ast_eval2(tree->left, variable1,variable2, value1,value2, type1,type2, num_of_fields1,num_of_fields2, flag,table1,table2) && ast_eval(tree->right, variable1,variable2, value1,value2, type1,type2, num_of_fields1,num_of_fields2, flag,table1,table2));
			}
